      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <ul className="list-disc list-inside space-y-2">
            ...
            <li><code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">icegraph snapshots &lt;table&gt;</code> — list a table's snapshot history as timestamp / id / operation, timestamps shown in your local timezone (<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">-j</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--json</code> returns the raw UTC values)</li>
            ...
            <li><code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">icegraph use &lt;table&gt; [--index N]</code> — with nothing, lists every numbered range you've loaded for this table (marking the current one); <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--index</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">-i</code> switches to a listed range by its number, without re-fetching</li>
        ...
        <div className="space-y-2">
          ...
          <p>
            <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--start</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--end</code> — only on <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">load</code>, never on <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">show</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">metadata</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">open</code>/<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">use</code> (which switches by <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--index</code> instead) — each accept a snapshot ID or a date/timestamp.
            ...
            Omitting them is handled differently for each, since they mean different things: omitting <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--start</code> stays
            unbounded (<code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">None</code>) — "from the beginning of history" is a valid, stable request on its own.
            Omitting <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--end</code> always resolves to a concrete number — the table's actual latest snapshot
            right now — never an open-ended marker, so a loaded range stays a fixed fact even if the table gets new snapshots later.
            A freshly created table with only its initial metadata file and no snapshots yet is still loadable — <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--start</code> stays
            unbounded as usual, and <code className="bg-surface-hover px-1.5 py-0.5 rounded text-[#7dd3fc] text-sm">--end</code> is recorded as empty; any later look at that range
            prints a note suggesting you load again once the table has data.
          </p>
          <p>
