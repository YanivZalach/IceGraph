  ...
  return (
    ...
    >
      <div className="flex justify-between items-center gap-2">
        <div
          className={`text-sm font-semibold ${isSelected ? "text-white" : "text-slate-200"}`}
        >
          {dateTime}
          {precision && (
            <span
              className={`font-normal ${isSelected ? "text-blue-200/70" : "text-slate-500"}`}
            >
              {precision}
            </span>
          )}
      ...
      <div
        className={`text-sm whitespace-nowrap ${isSelected ? "text-blue-200/70" : "text-slate-500"} mt-1`}
      >
        ID: {id}
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8">
      <div className="bg-surface rounded-2xl shadow-xl p-10 w-full max-w-6xl border border-edge">
        <h2 className={`${UI_PAGE_TITLE_CLASS} mb-4`}>Select Snapshots</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex gap-6">
            {latestSnapshotId && (
              <div className="w-44 shrink-0 flex flex-col justify-center items-center gap-3 border-r border-dashed border-edge pr-6">
                ...
                <p className={`${UI_HELPER_TEXT_CLASS} text-center`}>
                  Skip range selection and jump straight to the table's current
                  state
                </p>
              </div>
            )}
            <div className="flex flex-wrap flex-1 gap-6">
              <div className="flex-1 min-w-72">
                ...
                <div
                  ref={startListRef}
                  className="h-72 overflow-y-auto bg-edge rounded-xl p-2 space-y-2 scroll-py-4"
                >
          ...
          <button
            ref={submitBtnRef}
            type="submit"
            className="w-full bg-accent hover:bg-accent-dark py-3 rounded-lg text-white font-bold transition-colors"
          >
            Generate Graph
          </button>
        </form>
