...
export default function MetadataPage() {
  ...
  useEffect(() => {
    ...
    const scroll = (delta) => {
      ...
      scrollTargetRef.current = Math.max(
        0,
        Math.min(scrollTargetRef.current + delta, max),
      );
  ...
  const defaultSpec = metadata["partition-specs"]?.find(
    (s) => s["spec-id"] === metadata["default-spec-id"],
  );
  const defaultOrder = metadata["sort-orders"]?.find(
    (s) => s["order-id"] === metadata["default-sort-order-id"],
  );
  ...
  const currentSnapshotNode = (nodes || []).find(
    (n) =>
      n.type === FileType.SNAPSHOT &&
      n.details &&
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="group relative">
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-72 bg-surface text-slate-300 text-detail p-3 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 leading-relaxed">
              <strong className="text-accent block mb-1 uppercase tracking-wide text-base">
                Partial Metadata
              </strong>
              The following fields are stripped/altered by the backend due to
              size:
              <ul className="mt-1.5 flex flex-col gap-0.5">
                {[
                  ...
                  <li key={f} className="font-mono text-accent">
                    · {f}
                  </li>
                ))}
              </ul>
              ...
              <>
                <KV
                  ...
                  value={formatLocaleDateTime(
                    parseUtcDate(currentSnapshotNode.details.timestamp),
                  )}
                />
                ...
                <KV
                  ...
                  value={getStat("total-position-deletes")}
                />
                <KV
                  label="Equality Deletes"
                  value={getStat("total-equality-deletes")}
                />
              </>
            )}
          </div>
          ...
          <Section title="Refs">
            <div className="flex flex-col">
              {refs.map(([name, ref]) => (
                <div
                  key={name}
                  className="flex items-center gap-3 py-2 border-b border-edge last:border-0"
                >
                  <span className="text-sm font-semibold text-ink min-w-25">
                    {name}
                  </span>
                  <span className="text-xs font-bold uppercase px-2 py-0.5 rounded bg-edge text-slate-400">
                    {ref.type}
                  </span>
                  <span className={`${UI_MONO_MUTED_CLASS} ml-auto`}>
                    {ref["snapshot-id"]}
                  </span>
                </div>
              ))}
            ...
            <div className="flex flex-col">
              {properties.map(([k, v]) => {
                ...
                return (
                  <div
                    key={k}
                    className="flex items-start gap-4 py-2 border-b border-edge last:border-0"
                  >
                    <span className="text-sm font-mono text-accent min-w-45 shrink-0">
                      {k}
                    </span>
                    {parsedJson ? (
                      <JsonPropertyValue data={parsedJson} />
                    ) : (
                      <span className={`${UI_BODY_MUTED_CLASS} break-all`}>
                        {String(v)}
                      </span>
                    )}
                  </div>
