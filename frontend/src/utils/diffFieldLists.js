export function diffFieldLists(prevFields, currFields, idKeyFn) {
  const prevById = Object.fromEntries((prevFields ?? []).map(f => [idKeyFn(f), f]))
  const currById = Object.fromEntries((currFields ?? []).map(f => [idKeyFn(f), f]))
  ...
  return allIds.map(id => {
    ...
    if (JSON.stringify(prev) !== JSON.stringify(curr)) return { status: 'changed', prev, curr }
