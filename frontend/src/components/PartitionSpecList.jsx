...
function PartitionFieldHeader() {
  return (
    <div className="flex items-center gap-x-4 pb-1 mb-1 border-b border-edge">
      <span className="text-xs font-bold text-slate-500 uppercase w-10 text-right shrink-0">
        ID
      </span>
      <span className="text-xs font-bold text-slate-500 uppercase w-20 text-right shrink-0">
        Source ID
      </span>
      <span className="text-xs font-bold text-slate-500 uppercase min-w-30 shrink-0">
        Name
      </span>
...
export function PartitionFieldRow({ field, status, prevField }) {
  ...
  const transformChanged =
    status === "changed" && prevField.transform !== field.transform;

  return (
    <div
      className={`flex items-center gap-x-4 py-2 border-b border-edge last:border-0 ${bg} ${dim}`}
    >
      <span
        className={`text-base font-mono w-10 text-right shrink-0 tabular-nums ${idColor}`}
      >
        {marker ?? field["field-id"] ?? "—"}
      </span>
      <span
        className={`text-xs font-mono w-20 text-right shrink-0 tabular-nums ${sourceIdColor}`}
      >
        {field["source-id"] ?? "—"}
      </span>
      <span className={`text-sm font-semibold min-w-30 shrink-0 ${nameColor}`}>
        ...
        {status === "changed" && prevField.name !== field.name && (
          <span className="text-amber-600 line-through ml-2 font-normal">
            {prevField.name}
          </span>
        )}
      </span>
      {transformChanged ? (
        ...
        <span
          className={`text-xs font-mono shrink-0 ${status === "added" ? "text-green-400" : "text-red-400"}`}
        >
          {field.transform}
        </span>
      ) : status === "changed" ? (
        <span className="text-xs font-mono shrink-0 text-amber-300">
          {field.transform}
        </span>
      ) : (
...
export default function PartitionSpecList({ spec }) {
  ...
  return (
    <div>
      ...
      {spec.fields.map((f, i) => (
        <PartitionFieldRow key={f["field-id"] ?? i} field={f} />
      ))}
    </div>
  );
}
