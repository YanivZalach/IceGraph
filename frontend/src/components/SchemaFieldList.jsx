      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="ml-3 border-l-2 border-edge pl-4 flex flex-col gap-3 py-1">
          <div className="flex items-start gap-2">
            <span className="text-xs font-bold text-slate-500 uppercase mt-1 shrink-0 w-10 text-right">
              Key
            </span>
...
function SchemaFieldHeader() {
  return (
    <div className="flex items-center gap-4 pb-1 mb-1 border-b border-edge">
      ...
      <span className="text-xs font-bold text-slate-500 uppercase min-w-30">
        Name
      </span>
...
export function FieldRow({ field, status, prevField }) {
  ...
  const typeChanged =
    status === "changed" &&
    JSON.stringify(prevField.type) !== JSON.stringify(field.type);
...
export default function SchemaFieldList({ schema }) {
  ...
  return (
    <div>
      <SchemaFieldHeader />
      {schema.fields.map((f) => (
        <FieldRow key={f["field-id"] ?? f.name} field={f} />
      ))}
    </div>
  );
}
