    ...
    <nav className="bg-[#1a202c] text-white px-8 py-3 flex items-center gap-6 shadow-lg shrink-0">
      ...
      {!isTablePage && (
        <NavLink
          ...
          className={({ isActive }) =>
            `text-sm font-medium pb-0.5 border-b-2 transition ${isActive
      ...
      {isTablePage && (
        <>
          {tableName && (
            <button
              className={`text-sm font-mono px-3 py-1 rounded-md transition ${
                detailsOpen
          ...
          <button
            className="ml-auto text-sm font-medium text-slate-400 hover:text-white transition"
            onClick={() => navigate('/')}
          >
            Home
          </button>
