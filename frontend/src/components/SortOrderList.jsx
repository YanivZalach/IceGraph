...
function SortFieldHeader() {
  return (
    <div className="flex items-center gap-x-4 pb-1 mb-1 border-b border-edge">
      ...
      <span className="text-xs font-bold text-slate-500 uppercase w-20 shrink-0">
        Direction
      </span>
...
export function SortFieldRow({ field, status, prevField }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={
          nullOrderColor ? `text-sm ${nullOrderColor}` : UI_BODY_MUTED_CLASS
        }
      >
        {field["null-order"]}
        {status === "changed" &&
          prevField["null-order"] !== field["null-order"] && (
            <span className="text-amber-600 line-through ml-2">
              {prevField["null-order"]}
            </span>
          )}
      </span>
    </div>
  ...
  return (
    <div>
      ...
      {order.fields.map((f, i) => (
        <SortFieldRow key={f["source-id"] ?? i} field={f} />
      ))}
    </div>
  );
}
