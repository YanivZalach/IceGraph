...
def _add_range_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        "If omitted, stays unbounded (matches from the beginning of history).",
    )
    parser.add_argument(
        "--end",
        default=None,
        ...
        "If omitted, resolves to the table's actual latest snapshot right now.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--server", default=None, help="IceGraph server URL (env ICEGRAPH_SERVER_URL; prompted for and saved on first run if neither is set)"
    )
    parser.add_argument(
        "--data-dir", default=None, dest="data_dir", help=f"Local persistence directory (env ICEGRAPH_DATA_DIR, default {DEFAULT_DATA_DIR})"
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
    ...
    use_parser.add_argument(
        ...
        default=None,
        dest="index",
        help="Switch to the range at this index (see the numbered list from running `icegraph use <table>` with no flags)",
    )

    show_parser = subparsers.add_parser("show", help="Explore the currently loaded table's nodes")
    show_parser.add_argument("table")
    show_parser.add_argument("--type", choices=VALID_TYPES, default=None, help="Only list nodes of this type (`metadata` also includes `main_metadata`)")
    show_parser.add_argument("--operation", default=None, help="Only list snapshot nodes whose operation contains this text (case-insensitive)")
    ...
    show_mode_group.add_argument(
        "--node", metavar="NODE_ID", default=None, help="Show full details for one node (matches an exact id or a unique substring of its id/label)"
    )
    show_mode_group.add_argument(
        "--children",
        metavar="NODE_ID",
        default=None,
        help="List the children of one node (matches an exact id or a unique substring of its id/label)",
    )
    ...
    open_parser.add_argument("--page", choices=VALID_PAGES, default="graph", help="Which view to open (default: graph)")
